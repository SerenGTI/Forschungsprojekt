...
\todo{choose paragraph version}
Many parallel graph algorithms, including the three we consider here, are implemented by iterating over vertices \cite{tao}.
A vertex to which an operator is applied to is called an \emph{active} vertex.
Such an operator considers only the \emph{neighbourhood} of the \emph{active} vertex, where the \emph{neighbourhood} includes only adjacent vertices.
The locality of the operator simplifies the parallelisation of iterations over vertices.
