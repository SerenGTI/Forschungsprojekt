...
\begin{equation*}
	\operatorname{PR}(v_i) = \frac{1 - d}{|V|} + d \sum_{v_j \in N^-(v_i)} \frac{\operatorname{PR}(v_j)}{\operatorname{deg^+}(v_j)}
\end{equation*}
for all edges $v_i \in V$.
$d$ is the damping factor, $deg^+(v)$ is the number of outgoing edges the vertex $v$ has and $N^-(v)$ denotes the set of vertices that have edges to vertex $v$.
